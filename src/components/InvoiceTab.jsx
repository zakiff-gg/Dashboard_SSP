...
export default function InvoiceTab({ password }) {
  ...
  async function handleTambahPembayaran(inv) {
    const nominalStr = window.prompt(`Nominal pembayaran baru untuk ${inv.nomorInvoice || inv.id} (sisa ${formatRupiah(inv.nominal - inv.totalDibayar)}):`);
    ...
    try {
      ...
      if (res.success) {
        ...
        alert(res.message || "Gagal menyimpan pembayaran.");
      }
    } catch (err) {
      alert("Gagal terhubung ke server: " + err.message);
    } finally {
